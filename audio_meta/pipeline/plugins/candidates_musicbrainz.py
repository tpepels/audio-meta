...
class MusicBrainzCandidateSourcePlugin(CandidateSourcePlugin):
    ...
    def add(self, ctx: DirectoryContext) -> None:
        ...
        for pending in ctx.pending_results:
            ...
            ctx.release_scores[key] = max(ctx.release_scores.get(key, 0.0), float(pending.result.score))
            release_data = daemon.musicbrainz.release_tracker.releases.get(release_id)
            ctx.release_examples[key] = ReleaseExample(
                provider="musicbrainz",
                title=(release_data.album_title if release_data and release_data.album_title else pending.meta.album or "") or "",
                artist=(
                    release_data.album_artist if release_data and release_data.album_artist else pending.meta.album_artist or pending.meta.artist or ""
                )
                or "",
