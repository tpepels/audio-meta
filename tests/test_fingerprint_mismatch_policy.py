...
class TestFingerprintMismatchPolicy(unittest.TestCase):
    def _daemon(self, threshold: float = 0.35) -> AudioMetaDaemon:
        settings = Settings(
            library=LibrarySettings(roots=[Path("/music")], include_extensions=[".mp3"], exclude_patterns=[]),
            providers=ProviderSettings(acoustid_api_key="x", musicbrainz_useragent="test"),
            organizer=OrganizerSettings(enabled=False),
            daemon=DaemonSettings(fingerprint_mismatch_threshold=threshold, fingerprint_mismatch_action="defer"),
        )
    ...
    def test_conflict_when_album_and_artist_both_mismatch(self) -> None:
        ...
        meta = TrackMetadata(path=Path("/music/x.mp3"), album="Nocturnes", album_artist="Murray Perahia")
        self.assertTrue(daemon._fingerprint_conflicts_with_tags(tags, meta))

    def test_no_conflict_when_tags_missing(self) -> None:
        daemon = self._daemon()
        meta = TrackMetadata(path=Path("/music/x.mp3"), album="Album", album_artist="Artist")
